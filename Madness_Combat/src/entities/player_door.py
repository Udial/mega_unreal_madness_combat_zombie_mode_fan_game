...
class PlayerDoor:
    def __init__(
        self, cords_tuple: tuple, target_room: str, target_pos: tuple, wall_pos: str
    ):
        ...
        self.rect = pygame.Rect(min(xs), min(ys), self.width, self.height)
        ...
        self.player_interaction_rect = pygame.Rect(
            ...
            self.width
            if wall_pos == "top"
            else (self.width + 40 if wall_pos == "left" else self.width - 40),
            interaction_height,
        )

    def update(
        self, player: Player, collision_system: CollisionSystem, input_state: InputState
    ) -> bool:
        if (
            self.can_player_interact(player, collision_system)
            and input_state.interact_pressed
        ):
    ...
    def render(self, screen, camera: Camera = None):
        ...
        pygame.draw.polygon(screen, (140, 140, 140), points, 0)
