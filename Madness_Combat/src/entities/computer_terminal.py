...
class ComputerTerminal:
    def __init__(
        self, x: int, y: int, width: int, height: int, sprite_path: str | None = None
    ):
        self.rect = pygame.Rect(x, y, width, height)

        self.interaction_rect = pygame.Rect(
            x - 40, settings.ONE_FORTH_OF_SHOP_ROOM_HEIGHT * 3, width + 80, 80
        )
        self.can_interact = False

        self.sprite_path = sprite_path or settings.TERMINAL_SPRITE_PATH
        self.sprite = None
    ...
    def update(
        ...
        return self.can_interact and input_state.interact_pressed

    def can_player_interact(
        self, player: Player, collsion_system: CollisionSystem
    ) -> bool:
