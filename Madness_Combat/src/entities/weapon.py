...
class Weapon:
    def __init__(self, data: dict, combat_system: "CombatSystem"):
        ...
        self.sprite_width = data.get("sprite_width", data.get("sprite_widht", 48))
        ...
        self.original_sprite = pygame.transform.scale(
            self.original_sprite, (self.sprite_width, self.sprite_height)
        )
        self.sprite = self.original_sprite
        self.rect = self.sprite.get_rect()
        self.offset_from_center = data.get("offset", 0)
    ...
    def update(self, dt, camera=None):
        direction = self.get_direction(camera)
...
class Ranged(Weapon):
    def __init__(self, data: dict, combat_system: "CombatSystem"):
        ...
        self.muzzle_offset = data["muzzle_offset"]
        self.muzzle_flash_offset = pygame.Vector2(self.muzzle_offset)
        self.reloading = False
        self.reload_timer = 0
        self.muzzle_flash_timer = 0
        self.muzzle_flash_sprite = self._load_muzzle_flash()
        self.muzzle_world_pos = pygame.Vector2(self.rect.center)

    def _load_muzzle_flash(self):
        try:
            sprite = pygame.image.load(
                settings.MUZZLE_FLASH_SPRITE_PATH
            ).convert_alpha()
            return pygame.transform.scale(sprite, (54, 32))
        except Exception:
            surf = pygame.Surface((54, 32), pygame.SRCALPHA)
            pygame.draw.polygon(
                surf, (255, 220, 90, 220), [(0, 16), (54, 4), (42, 16), (54, 28)]
            )
            return surf
    ...
    def get_muzzle_world_pos(self):
        ...
        local_offset = pygame.Vector2(self.muzzle_flash_offset)

        if direction.x < 0:
            local_offset.y *= -1

        rotated_offset = local_offset.rotate(angle)

        return pygame.Vector2(self.owner.rect.center) + rotated_offset

    def update_ranged(self, dt, camera=None):
        super().update(dt, camera)
        self.muzzle_world_pos = self.get_muzzle_world_pos()
    ...
    def can_shoot(self):
        return self.cooldown <= 0 and self.current_ammo > 0 and not self.reloading

    def shoot(self):
        if not self.can_shoot():
            return
        origin = self.get_muzzle_world_pos()
        for _ in range(self.pellets):
            self.combat_system.spawn_bullet(
                self.damage,
                self.spread,
                self.bullet_speed,
                self.pierce,
    ...
    def render(self, screen, camera=None):
        ...
        angle = -math.degrees(math.atan2(direction.y, direction.x))
        flash = self.muzzle_flash_sprite
        if direction.x < 0:
            flash = pygame.transform.flip(flash, False, True)
        flash = pygame.transform.rotate(flash, angle)
        rect = flash.get_rect(center=self.muzzle_world_pos)
        if camera is not None:
