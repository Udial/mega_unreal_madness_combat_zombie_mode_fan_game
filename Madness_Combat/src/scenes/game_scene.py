...
class GameScene(BaseScene):
    def __init__(self, game):
        ...
        self.shop_room_door = PlayerDoor(
            settings.ENTRY_POINTS_TUPLE[2], "shop_room", (120, 850), "top"
        )

        shop_exit_points = make_wall_entry(
            settings.WALL_CORDS_TUPLE_SHOP[0], u1=0.4, u2=0.6, v_top=0.8, v_bottom=1
        )

        self.shop_room_exit_door = PlayerDoor(
            shop_exit_points, "arena", self.shop_room_door.rect.center, "left"
        )

        self.window_entry_left = EntryPoint(settings.ENTRY_POINTS_TUPLE[0], "top", 0)

        self.window_entry_right = EntryPoint(settings.ENTRY_POINTS_TUPLE[1], "top", 1)

        left_door_points = make_wall_entry(
            settings.WALL_CORDS_TUPLE[0], u1=0.4, u2=0.6, v_top=0.8, v_bottom=1
        )
        right_door_points = make_wall_entry(
            settings.WALL_CORDS_TUPLE[1], u1=0.4, u2=0.6, v_top=0.8, v_bottom=1
        )

        self.left_door_entry = EntryPoint(left_door_points, "left", 2)
        self.right_door_entry = EntryPoint(right_door_points, "right", 3)
        ...
        self.shop_terminal = ComputerTerminal(
            settings.SHOP_ROOM_WIDTH_MID - (128 / 2),
            settings.ONE_FORTH_OF_SHOP_ROOM_HEIGHT * 3 - 158,
            128,
            128,
        )

        self.player = Player(
            800, 900, settings.HITBOX_WIDTH, settings.HITBOX_HEIGHT, 500, 100
        )
        ...
        self.shop_manager = ShopManager(
            self.economy_manager, self.player, self.weapon_factory
        )
        self.collision_system = CollisionSystem(self.walls, self.shop_walls)
        self.input_system = InputSystem()
        self.movement_system = MovementSystem(self.collision_system)
        self.spawn_manager = SpawnManager(
            self.entry_points, self.zombie_list, self.weapon_factory
        )
        self.wave_manager = WaveManager(
            self.spawn_manager, self.zombie_list, self.entry_points
        )
        ...
        self.shop_ui = ShopUI(self.shop_manager)
    ...
    def update(self, dt):
        ...
        if self.player.hp <= 0:
            ...
            self.game.scene_manager.set_scene(GameOverScene(self.game))
            return

        if self.wave_manager.all_waves_completed:
            from .win_scene import WinScene

            self.game.scene_manager.set_scene(WinScene(self.game))
            return
        ...
        if self.current_room == "arena":
            ...
            self.wave_manager.update(dt, input_state)
            ...
            if self.shop_terminal.update(
                self.player, self.collision_system, input_state
            ):
                self.shop_ui.open()
        ...
        self.movement_system.update(self.player, input_state, self.current_room, dt)

        self.camera.update(self.player)

        for ep in self.entry_points:
            ep.update(
                self.player, self.collision_system, input_state, self.damage_system, dt
            )
    ...
    def render(self, screen):
        ...
        if self.current_room == "arena":
            ...
            self.shop_room_door.render(screen, self.camera)
            ...
            self.shop_room_exit_door.render(screen, self.camera)

            self.shop_terminal.render(screen, self.camera)
            self.shop_terminal.render_hint(screen, self.camera)
