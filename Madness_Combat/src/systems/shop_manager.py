...
class ShopManager:
    ...
    def buy_item(self, category: str, item: dict) -> bool:
        ...
        if item.get("type") == "barricade" or item.get("id") == "barricade":
            self.buy_barricade(item)
    ...
    def buy_armor(self, item: dict):
        if "charge" in item and self.player.armor < self.player.max_armor:
            self.player.armor = int(
                min(self.player.max_armor, self.player.armor + item["charge"])
            )

    def buy_barricade(self, item: dict):
        self.player.amount_of_barricades += item.get("amount", 1)
